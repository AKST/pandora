defmodule Pandora.Parse do
  ...
  defprotocol Parser do
    ...
    @spec slice_from_cursor_offset(parser :: any, offset :: pos_integer(), amount :: pos_integer()) :: {any, String.t()}
  ...
  defp maybe_parse_doctype(p) do
    ...
    parse_inlined_dtd_impl = fn (f, p, chunks, offset) ->
      case Parser.slice_from_cursor_offset(p, offset, 1) do
        ...
        {p, _} -> case Parser.slice_from_cursor_offset(p, offset, 4) do
          {p, "<!--"} ->
    ...
    case slice_from_cursor(p, 2) do
      {p, "<!"} -> case Parser.slice_from_cursor_offset(p, 2, 2) do
        {p, "--"} ->
          {:ok, {p, nil}}
    ...
    case Parser.lookup_from_cursor(p, 0 + start_from) do
      ...
      {p, "<"} ->
        case Parser.lookup_from_cursor(p, 1 + start_from) do
          {p, "!"} -> case Parser.lookup_from_cursor(p, 2 + start_from) do
            {p, "-"} -> case Parser.lookup_from_cursor(p, 2 + start_from) do
              {p, "-"} -> {:ok, {p, :comment}}
              {p, _} -> {:ok, {p, :text}}
            end
  ...
  @spec are_these_the_next_chunks(p, chunks :: nonempty_list(String.t), offset :: non_neg_integer) :: {p, boolean, non_neg_integer} when p: any
  ...
  @spec amount_til_end_of_whitespace(parser :: any, index :: non_neg_integer) :: {any, non_neg_integer}
  ...
  @spec consume_while(p, predicate :: (String.t -> boolean), index :: non_neg_integer) :: {p, String.t} when p: any
